...
impl Parser {
    pub fn parse_current_semester(html: &str) -> Option<(i32, String)> {
        ...
        let extract_from_text = |text: &str| -> Option<(i32, String)> {
            ...
            // 放宽对结尾分号的限制，并使用非贪婪匹配
            let re_obj = RE_OBJ.get_or_init(|| Regex::new(r"(?s)var\s+currentSemester\s*=\s*(\{.*?\})").unwrap());
            
            static RE_JSON: OnceLock<Regex> = OnceLock::new();
            // 匹配 JSON.parse('...') 中的内容，支持单引号或双引号包裹
            // 注意：原始字符串中双引号不需要转义，但单引号和双引号在正则字符类中可以直接写
            let re_json = RE_JSON.get_or_init(|| Regex::new(r#"(?s)var\s+currentSemester\s*=\s*JSON\.parse\(\s*['"](.*?)['"]\s*\)"#).unwrap());

            // 辅助函数：尝试解析 JSON 字符串
            let try_parse_json = |json_str: &str| -> Option<(i32, String)> {
                // 1. 标准解析
                if let Ok(sem) = serde_json::from_str::<CurrentSemester>(json_str) {
                    info!("[RustSDKSchedule] JSON parse success: id={}, name={}", sem.id, sem.name);
                ...
                if let Ok(sem) = serde_json::from_str::<CurrentSemester>(&fixed_json) {
                    info!("[RustSDKSchedule] Fixed JSON parse success: id={}, name={}", sem.id, sem.name);
                    return Some((sem.id, sem.name));
                }
                ...
                if let Ok(sem) = serde_json::from_str::<CurrentSemester>(&single_quote_fixed) {
                     info!("[RustSDKSchedule] Single-quote fixed JSON parse success: id={}, name={}", sem.id, sem.name);
                     return Some((sem.id, sem.name));
                }
                ...
                warn!("[RustSDKSchedule] JSON parsing failed. Content: {}", json_str);
                ...
                let id_re = ID_RE.get_or_init(|| Regex::new(r#"(?:id|'id'|"id")\s*:\s*(\d+)"#).unwrap());
                ...
                if let Some(cap) = id_re.captures(json_str) {
                     if let Ok(id) = cap[1].parse::<i32>() {
                         ...
                         // 支持 name: '...', name: "...", "name": "..."
                         // 还要支持单引号 key: 'name':'...'
                         let name_re = NAME_RE.get_or_init(|| Regex::new(r#"(?:name|'name'|"name")\s*:\s*['"]([^'"]+)['"]"#).unwrap());
                         ...
                         info!("[RustSDKSchedule] Regex extraction from snippet success: id={}, name={}", id, name);
                         return Some((id, name));
                     }
                }
            ...
            if let Some(start_idx) = text.find("var currentSemester") {
                ...
                if let Some(cap) = id_re.captures(snippet) {
                     if let Ok(id) = cap[1].parse::<i32>() {
                         // 匹配 name: "..." 或 "name": "..."
                         static NAME_RE: OnceLock<Regex> = OnceLock::new();
                         let name_re = NAME_RE.get_or_init(|| Regex::new(r#"(?:name|"name")\s*:\s*"([^"]+)""#).unwrap());
                         ...
                         info!("[RustSDKSchedule] Regex fallback extraction success: id={}, name={}", id, name);
                         return Some((id, name));
                     }
                }
            } else {
        ...
        let script_re = SCRIPT_RE.get_or_init(|| Regex::new(r"(?s)<script[^>]*>(.*?)</script>").unwrap());
        ...
        for cap in script_re.captures_iter(html) {
            ...
            if content.contains("var semesters") && content.contains("var currentSemester") {
                script_found = true;
                info!("[RustSDKSchedule] Found target script block (len: {})", content.len());
                // 打印一部分内容以便调试
                let preview_len = std::cmp::min(content.len(), 500);
                info!("[RustSDKSchedule] Script Content Head: {}", &content[..preview_len]);
                     ...
                     if let Some(start_var) = start_search_text.find("var currentSemester") {
                         ...
                         if let Some(start_brace) = after_var.find('{') {
                             ...
                             if found {
                                 ...
                                 // 尝试标准解析
                                 if let Ok(sem) = serde_json::from_str::<CurrentSemester>(json_str) {
                                     info!("[RustSDKSchedule] Balanced JSON parse success: id={}, name={}", sem.id, sem.name);
                                     return Some((sem.id, sem.name));
                                 }
                                 ...
                                 let single_quote_fixed = json_str.replace("'", "\"");
                                 if let Ok(sem) = serde_json::from_str::<CurrentSemester>(&single_quote_fixed) {
                                      info!("[RustSDKSchedule] Balanced Single-quote fixed JSON parse success: id={}, name={}", sem.id, sem.name);
                                      return Some((sem.id, sem.name));
                                 }
                                 ...
                                 if let Some(cap) = id_re.captures(json_str) {
                                      if let Ok(id) = cap[1].parse::<i32>() {
                                          ...
                                          info!("[RustSDKSchedule] Balanced JSON Regex extraction success: id={}, name={}", id, name);
                                          return Some((id, name));
                                      }
                                 }
                             } else {
                                 warn!("[RustSDKSchedule] Failed to find balanced closing brace for currentSemester.");
                             }
                         }
                     }
                     None
        ...
        error!("[RustSDKSchedule] All parsing attempts failed. HTML len: {}", html.len());
    ...
    pub fn parse_semesters_list(html: &str) -> Option<String> {
        ...
        let re = RE.get_or_init(|| Regex::new(r"(?s)var\s+semesters\s*=\s*JSON\.parse\(\s*'(.*?)'\s*\);").unwrap());
    ...
    pub fn parse_cas_params(html: &str) -> Option<(String, String, String)> {
        ...
        let action_re = ACTION_RE.get_or_init(|| Regex::new(r#"<form[\s\S]*?action="([^"]+)""#).unwrap());
        ...
        let mut action = action_re.captures(html)
            .map(|c| c[1].to_string())
            .unwrap_or_default();
